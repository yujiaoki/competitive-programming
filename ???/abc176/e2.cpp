...
int main(void){
    // Your code here!
    int H,W,m; cin >> H >> W >> m;
    map<int,int,greater<int>> mph,mpw;
    set<pair<int,int>> st;
    ...
    int ret = 0;
    int mh = MAX(vh),mw = MAX(vw);
    vector<int> gh,gw;
    rep(i,H) if (vh[i] == mh) gh.push_back(i);
    rep(i,W) if (vw[i] == mw) gw.push_back(i);
    if (ll(gh.size()) * gw.size() > m) {
        cout << mh+mw << endl;
        return 0;
    }
    for(auto h:gh) for (auto w:gw) 
    {
        if (!st.count({h,w})) {
            cout << mh + mw << endl;
            return 0;
