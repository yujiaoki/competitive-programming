...
int main(void){
    ...
    UnionFind tree(n);
    vector<int> ans(n,0);
    vector<vector<int>> Ga(n,vector<int>(0));
    ...
    vector<vector<int>> Gb(n,vector<int>(0));
    ...
    vector<int> root_num(n,0);
    rep(i,n){
        root_num[tree.root(i)]++;
    }

    rep(i,n){
        ans[i] += root_num[tree.root(i)];
        ans[i]--;
        ans[i]-=Ga[i].size();
        for(auto nv:Gb[i]){
            if(tree.same(i,nv)) ans[i]--;
        }
    }
    rep(i,n){
        cout << ans[i] << " ";
