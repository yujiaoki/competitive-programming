...
int main(void){
    ...
    rep(i,31) {
        rep(j,31){
            ll water = 100*(i*a+j*b);
            ll maxsugar = (i*a+j*b)*e;
            if(water>f) continue;
            rep(k,maxsugar/c+1) {
                rep(l,(maxsugar-k*c)/d+1){
                    if(water==0) break;
                    ll sugar = k*c+l*d;
                    if(sugar > maxsugar) break; 
                    if(water+sugar>f) break;
                    if(den < (long double)sugar/(sugar+water)){
                        den = (long double)sugar/(sugar+water);
                        ans1 = sugar+water;
                        ans2 = sugar;
                    }
                }
