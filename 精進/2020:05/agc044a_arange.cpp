...
ll n,a,b,c,d;
int t;
ll dfs(ll n,ll cur, ll res, ll &ans,ll &count){
    cout << n << " " << cur << " " << res << " " << ans << " " <<count << endl;
    // return;
    if(n == cur) {
        ans = min(ans,res);
        return ans;
    ...
    if (cur < 2*n) dfs(n,cur*2,res+a,ans,++count);
    if (cur < 3*n) dfs(n,cur*3,res+b,ans,++count);
    if (cur < 5*n) dfs(n,cur*5,res+c,ans,++count);
    dfs(n,cur-1,res+d,ans,++count);
    dfs(n,cur+1,res+d,ans,++count);
}
int main(void){
    // Your code here!
