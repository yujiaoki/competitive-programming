...
char dfs(ll i,int k) {
    ...
    if (i == 1) return 'A';
    if (i > 1 && i <= 1+counter(k-1)) {
        return dfs(i-1,k-1);
    } else if (i == counter(k-1)+2) return 'B';
    else if (i > counter(k-1)+2 && i < counter(k)-1) {
        return dfs(i-2-counter(k-1),k-1) ;
    } else {
        return 'C';
    }
}
