...
int main(void){
    ...
    if (n % 2 == 1) {
        ...
        front = round((double)aa[(n-1)/2]/2+(double)aa[(n)/2]/2);
        back = round((double)bb[(n-1)/2]/2+(double)bb[(n)/2]/2);
    }
    ll ans = 0;
    rep(i,n) {
        if ( a[i] < front) ans+= 2*abs(a[i]-front);
        ans += back - front;
        if (back < b[i]) ans += 2*abs(b[i]-back);
    }
    cout << ans << endl;
}
