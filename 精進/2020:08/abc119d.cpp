...
ll solve(ll x, ll s,ll t) {
    if (s > t) swap(s,t);
    ll ans = 0;
    if (x < s) {
        ans += t-x;
    } else if (s <= x && x <= t) {
        ans += min(x-s+ t- s,t-x+t-s);
    } else {
        ans = x-s;
    }
    return ans;
...
int main(void){
    ...
    rep(m,q) {
        ...
        ll j = UPPER(t,x);
        ll ans = 100000000000;
        if (i != a && j != b) ans = min(ans,solve(x,s[i],t[j]));
        // cout << i << " " << j << " " << ans << endl;
        i = UPPER(s,x);
        j = UPPER(t,x) - 1;
        if (i != a && j != -1) ans = min(ans,solve(x,s[i],t[j]));
        // cout << i << " " << j << " " << ans << endl;
        i = UPPER(s,x) - 1;
        j = UPPER(t,x);
        if (j != b && i != -1) ans = min(ans,solve(x,s[i],t[j]));
        // cout << i << " " << j << " " << ans << endl;
        i = UPPER(s,x) - 1;
        j = UPPER(t,x) - 1;
        if (i != -1 && j != -1)ans = min(ans,solve(x,s[i],t[j]));
        // cout << i << " " << j << " " << ans << endl;
        cout << ans << endl;
    }
}
